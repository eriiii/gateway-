...
func read(listen *net.TCPConn, ch chan []byte) {
	defer wg.Done()
	for {
		buf := make([]byte, 2048)
...
func main() {
	...
	tcpAddr, _ := net.ResolveTCPAddr("tcp", "192.168.0.199:14567")

	listen1, err := net.ListenTCP("tcp", tcpAddr)
	if err != nil {
		fmt.Println("listen tcp err = ", err)
		return
	}

	conn1, err := listen1.AcceptTCP()
	if err != nil {
		fmt.Println("listen err = ", err)
		return
	}

	//set reading buffer length
	err = conn1.SetReadBuffer(512 * 1024 * 1024)
	if err != nil {
		fmt.Println("set buffer err = ", err)
		return
	}

	//server gateway address, open socket to send to server
